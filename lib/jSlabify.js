			...
			if(keepSpans) {  //build array of sections to later slab
				...
				while(a=div.querySelectorAll("span.slabbedtext")[0]) {  // continues grabbing all span.slabbedtext tags until none are left:
					var groupedValue    = $(a).text(),
							rawGroupedValue = a.outerHTML;
					if(settings.wrapAmpersand) {
						groupedValue = groupedValue.replace(/&/g, "<span class=\"amp\">&amp;</span>"); // preserved for legacy
						rawGroupedValue = rawGroupedValue.replace(/<span class="amp">&amp;<\/span>/g, "&amp;"); // in case we already wrapped the ampersands
						rawGroupedValue = rawGroupedValue.replace(/&amp;/g, "<span class=\"amp\">&amp;</span>");
					}
					...
					if(settings.wrapAmpersand) {
						ungroupedValue = ungroupedValue.replace(/&amp;/g, "<span class=\"amp\">&amp;</span>")
					}
					...
					sections.push({type: 1, value: groupedValue, raw: rawGroupedValue });			// push the span code to the stack
					$(a).prevAll().remove();
					$(a).remove();
				}
				if(remainder != "") {
				...
				if(forceNewCharCount || fs != origFontSize) {
					...
					function makeSlabSpans(charPerLine, sectionWords) {
						...
						while (wordIndex < sectionWords.length) {
							...
							finalText = $('<div></div>').text(finalText).html();
